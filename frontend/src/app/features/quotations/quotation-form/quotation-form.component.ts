...
@Component({
  ...
  template: `
    <app-page-header [title]="isEdit ? 'Edit Quotation' : 'New Quotation'"
                     [subtitle]="isEdit ? 'Update estimation details.' : 'Create a new price estimate.'">
      <a routerLink="/quotations" class="btn-sip-secondary">
        ...
        <div class="col-12">
          <form [formGroup]="form" (ngSubmit)="onSubmit()">
              ...
              <div class="col-12 col-lg-3">
                <div class="sip-card h-100">
                  ...
                  <div class="form-group">
                    ...
                    @if (f['customerId'].invalid && f['customerId'].touched) {
                      <span class="form-error">Required</span>
                    }
                  </div>
                  <div class="form-group">
                    <label class="form-label">Date *</label>
                    <input class="sip-input" type="date" formControlName="quotationDate">
                  </div>
                  <div class="form-group">
                    <label class="form-label">Valid Until *</label>
                    <input class="sip-input" type="date" formControlName="validUntil">
                  </div>
                  <div class="form-group">
                  ...
                  <div class="form-group">
                    ...
                    <textarea class="sip-textarea" formControlName="notes" rows="3" placeholder="Customer notes..."></textarea>
                  </div>
                </div>
              </div>
                      ...
                      <tbody formArrayName="lines">
                        @for (line of lines.controls; track i; let i = $index) {
                          <tr [formGroupName]="i">
                            ...
                            <td>
                              <input class="sip-input py-1 text-center" type="number" min="1" formControlName="quantity">
                            </td>
                            <td>
                              <input class="sip-input py-1 text-end" type="number" step="0.01" formControlName="unitPrice">
                            </td>
                            <td>
                              <input class="sip-input py-1 text-center" type="number" step="0.1" formControlName="remise">
                            </td>
                            <td>
                              <input class="sip-input py-1 text-center" type="number" step="0.1" formControlName="tva">
                            </td>
                            <td class="text-end fw-semibold" style="vertical-align:middle">
                              {{ calculateLineTotal(i) | currency:'MAD ':'symbol':'1.2-2' }}
                            </td>
                        ...
                        @if (lines.length === 0) {
                          <tr>
                            <td colspan="7" class="text-center text-muted py-4">No lines added yet.</td>
                          </tr>
                        }
                      </tbody>
                    </table>
                  ...
                  <div class="mt-4" style="display:flex;justify-content:flex-end">
                    <div style="width:300px;background:var(--bg-elevated);border:1px solid var(--border-color);border-radius:8px;padding:16px">
                      <div class="d-flex justify-content-between mb-2 fs-13 text-secondary">
                        <span>Total HT</span>
                        <span>{{ totalHT() | currency:'MAD ':'symbol':'1.2-2' }}</span>
                      </div>
                      <div class="d-flex justify-content-between mb-2 fs-13 text-secondary">
                        <span>Total Discount</span>
                        <span>-{{ totalDiscount() | currency:'MAD ':'symbol':'1.2-2' }}</span>
                      </div>
                      <div class="d-flex justify-content-between mb-2 fs-13 text-secondary">
                        <span>Total TVA</span>
                        <span>{{ totalTVA() | currency:'MAD ':'symbol':'1.2-2' }}</span>
                      </div>
            ...
            <div class="mt-4" style="display:flex;justify-content:flex-end;gap:10px">
              <a routerLink="/quotations" class="btn-sip-secondary">Cancel</a>
...
export class QuotationFormComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    quotationDate: [this.today(), Validators.required],
    validUntil:    [this.todayPlusDays(30), Validators.required],
    status:        ['DRAFT'],
    notes:         [''],
    lines:         this.fb.array([])
  });
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      c: this.custSvc.findAllList(),
      p: this.prodSvc.findAllList()
    }).subscribe({
      next: ({ c, p }) => {
        ...
        if (this.isEdit) {
          this.svc.findById(this.id!).subscribe(q => {
    ...
    // React to form changes to update computed signals
  ...
  onProductChange(i: number): void {
    ...
    const pid = line.get('productId')?.value;
    if (pid) {
      const p = this.products().find(x => x.id === pid);
      if (p) {
        line.patchValue({ unitPrice: p.prix, tva: p.tva, remise: 0 });
      }
    }
  }

  calculateLineTotal(i: number): number {
    const l = this.linesValue()[i];
    if (!l) return 0;
    const gross = l.quantity * l.unitPrice;
    const net = gross * (1 - l.remise / 100);
    return net * (1 + l.tva / 100);
  }

  totalHT = computed(() => {
    return this.linesValue().reduce((acc, l) => acc + (l.quantity * l.unitPrice), 0);
  });

  totalDiscount = computed(() => {
    return this.linesValue().reduce((acc, l) => {
      const gross = l.quantity * l.unitPrice;
      return acc + (gross * (l.remise / 100));
    }, 0);
  });
  ...
  totalTVA = computed(() => {
    return this.linesValue().reduce((acc, l) => {
      const gross = l.quantity * l.unitPrice;
      const net = gross * (1 - l.remise / 100);
      return acc + (net * (l.tva / 100));
    }, 0);
  });

  totalTTC = computed(() => {
    return this.totalHT() - this.totalDiscount() + this.totalTVA();
  });
  ...
  onSubmit(): void {
    ...
    if (payload.quotationDate && payload.quotationDate.length === 10) payload.quotationDate += 'T00:00:00';
    if (payload.validUntil && payload.validUntil.length === 10) payload.validUntil += 'T00:00:00';
    ...
    req.subscribe({
      next: () => {
        this.notify.success('Success', `Quotation ${this.isEdit ? 'updated' : 'created'}.`);
  ...
  private todayPlusDays(d: number): string {
    const dt = new Date();
