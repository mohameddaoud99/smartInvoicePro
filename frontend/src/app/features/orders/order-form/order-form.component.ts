...
@Component({
  ...
  template: `
    <app-page-header [title]="isEdit ? 'Edit Order' : 'New Order'"
                     [subtitle]="isEdit ? 'Update order details.' : 'Create a new customer order.'">
      <a routerLink="/orders" class="btn-sip-secondary">
        ...
        <div class="col-12">
          <form [formGroup]="form" (ngSubmit)="onSubmit()">
              ...
              <div class="col-12 col-lg-3">
                <div class="sip-card h-100">
                  ...
                  <div class="form-group">
                    ...
                    <input class="sip-input" type="date" formControlName="orderDate">
                  </div>
                  <div class="form-group">
                  ...
                  <div class="form-group">
                    ...
                    <textarea class="sip-textarea" formControlName="deliveryAddress" rows="2"></textarea>
                  </div>
                  <div class="form-group">
            ...
            <div class="mt-4 d-flex justify-content-end gap-2">
              <a routerLink="/orders" class="btn-sip-secondary">Cancel</a>
...
export class OrderFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    forkJoin({ c: this.custSvc.findAllList(), p: this.prodSvc.findAllList() }).subscribe({
      next: ({ c, p }) => {
        ...
        if (this.isEdit) {
          this.svc.findById(this.id!).subscribe(o => {
            this.form.patchValue({
              customerId: o.customerId, orderDate: o.orderDate,
              status: o.status
            });
            o.lines.forEach(l => {
  ...
  onSubmit(): void {
    ...
    if (payload.orderDate && payload.orderDate.length === 10) payload.orderDate += 'T00:00:00';

    const req = this.isEdit ? this.svc.update(this.id!, payload) : this.svc.create(payload);
    req.subscribe({
      next: () => { this.notify.success('Success', `Order saved.`); this.router.navigate(['/orders']); },
